...
register(
        id='MoveoIK_discrete-v0',
        entry_point='moveo_inverse_kinematic_discrete:MoveoIKEnv',
...
class MoveoIKEnv(moveo_env.MoveoEnv, utils.EzPickle):
    def __init__(self):
        ...
        self.number_actions = (270, 270, 270, 270 ,270)
        self.action_space = spaces.Discrete(np.prod(self.number_actions))
    ...
    def _set_action(self, action):
        mapping = tuple(np.ndindex(self.number_actions))
        action = mapping[action]
        self.new_pos = {
                "Joint_1": float(action[0]/180*math.pi),
                "Joint_2": float(action[1]/180*math.pi),
                "Joint_3": float(action[2]/180*math.pi),
                "Joint_4": float(action[3]/180*math.pi),
                "Joint_5": float(action[4]/180*math.pi)
                }
