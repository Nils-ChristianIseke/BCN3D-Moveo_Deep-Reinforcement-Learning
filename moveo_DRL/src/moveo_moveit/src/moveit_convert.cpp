...
int stepsPerRevolution[6] = {200,200,32000,16000,3200,0};  // microsteps/revolution (using 16ths) from observation, for each motor
int joint_status = 0;
double cur_angle[6];
int joint_step[6];
...
void cmd_cb(const sensor_msgs::JointState& cmd_arm)
{
  ...
  ROS_INFO_NAMED("test", "arm_steps.position5 #2: %f", arm_steps.position[5]);
  ...
  total.position[0] += arm_steps.position[1];
  total.position[1] += arm_steps.position[2];
  total.position[2] += arm_steps.position[3];
  total.position[3] += arm_steps.position[4];
  total.position[4] += arm_steps.position[5];

  ROS_INFO_NAMED("test", "total_steps[4]: %f, total: %f", total_steps[4], total.position[5]);
